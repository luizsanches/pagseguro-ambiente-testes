...
{
	/***** SETTINGS *****/
    private $notification_domain = 'localhost:3000';
    private $notification_page = '/pagseguro/notification/';
    ...
    private $required_data = array("currency", "email", "itemId1", "itemDescription1", "itemQuantity1", "itemAmount1");
    ...
    public function sendNotification($transactionStatus, $notificationType) {
    	$this->notification = array(
            "notificationCode" => $this->generateRandomString(self::NOTIFICATION_CODE_LENGTH),
    ...
    private function generateTransaction() {
		...
		// sender
		$xml->sender->name = isset($this->order['cliente_nome']) ?: "Mauro Turm";
		$xml->sender->email = isset($this->order['cliente_email']) ?: "mauro@mail.com";
		$xml->sender->phone->areaCode = isset($this->order['cliente_ddd']) ?: "31";
		$xml->sender->phone->number = isset($this->order['cliente_tel']) ?: "55555555";

		// shipping
		$xml->shipping->address->street = isset($this->order['cliente_end']) ?: "Av. do Contorno";
		$xml->shipping->address->number = isset($this->order['cliente_num']) ?: "500";
		$xml->shipping->address->complement = isset($this->order['cliente_compl']) ?: "2o Andar";
		$xml->shipping->address->district = isset($this->order['cliente_bairro']) ?: "Centro";
		$xml->shipping->address->postalCode = isset($this->order['cliente_cep']) ?: "30110039";
		$xml->shipping->address->city = isset($this->order['cliente_cidade']) ?: "Belo Horizonte";
		$xml->shipping->address->state = isset($this->order['cliente_uf']) ?: "MG";
		$xml->shipping->address->country = isset($this->order['cliente_pais']) ?: "BRA";
		$xml->shipping->type = 3;
    ...
    private function updateTransaction($notification) {
		...
		if ($notification['transactionStatus'] == 7)
            $xml->cancellationSource = "INTERNAL";
